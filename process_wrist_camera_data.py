#!/usr/bin/env python3
"""
Script to process wrist camera data from a dataset folder.
Extracts intrinsics, extrinsics, and stereo frames for the wrist camera only.

Usage:
    python process_wrist_camera_data.py /path/to/dataset/folder
"""
...
def process_wrist_camera_data(input_dir, output_dir):
    """
    Process wrist camera data from the input directory.
    Extracts intrinsics, extrinsics, and stereo frames for the wrist camera only.
    """
    ...
    print(f"\n{'='*60}")
    print("Wrist camera processing complete!")
...
def main():
    parser = argparse.ArgumentParser(
        description='Process wrist camera data: extract intrinsics, extrinsics, and stereo frames'
    )
    parser.add_argument(
        '--input_dir',
        type=str,
    ...
    try:
        process_wrist_camera_data(args.input_dir, args.output_dir)
    except Exception as e:
        print(f"Error: {e}")
        return 1
